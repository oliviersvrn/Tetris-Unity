...
public class GridController : MonoBehaviour
{
    ...
    public void SetBlock(Vector2[] blockList, Color color, bool onlyColor, bool joint = true)
    {
        ...
        {
            SetBlock(blockList[i], color, onlyColor);
    ...
    public void SetBlock(Vector2[] blockList, Color[] colorList, bool onlyColor)
    {
        for (int i = 0; i < blockList.Length && i < colorList.Length; i++) {
            SetBlock(blockList[i], colorList[i], onlyColor);
        }
    ...
    public void SetBlock(Vector2 block, Color color, bool onlyColor)
    {
        if (block.x >= 0 && block.x < 10 && block.y >= 0 && block.y < 20)
        {
            MeshRenderer renderer = grid_objects[(int)block.x, (int)block.y].GetComponent<MeshRenderer>();
            renderer.enabled = true;
            renderer.material.SetColor("_Color", color);
    ...
    public void UnsetBlock(Vector2[] blockList, bool onlyColor)
    {
        for (int i = 0; i < blockList.Length; i++) {
            UnsetBlock(blockList[i], onlyColor);
        }
    }

    public void UnsetBlock(Vector2 block, bool onlyColor)
    {
        if (block.x >= 0 && block.x < 10 && block.y >= 0 && block.y < 20)
        {
            if (onlyColor && grid[(int)block.x, (int)block.y] == 1)
                return;

            grid_objects[(int)block.x, (int)block.y].GetComponent<MeshRenderer>().enabled = false;
    ...
    public void MoveDown(int y)
    {
        for (int j = y; j < 19; j++)
        {
            ...
            {
                grid[i,j] = grid[i,j+1];
                grid_objects[i,j].GetComponent<MeshRenderer>().enabled = grid_objects[i,j+1].GetComponent<MeshRenderer>().enabled;
                grid_objects[i,j].GetComponent<MeshRenderer>().material.color = grid_objects[i,j+1].GetComponent<MeshRenderer>().material.color;
        ...
        {
            grid[i,19] = 0;
            grid_objects[i,19].GetComponent<MeshRenderer>().enabled = false;
            if (i < 9)
